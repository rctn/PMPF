...
function Samples=sample_grbm_PT_beta_0610(Weights_HbV_ini, VBias_Vb1_ini, HBias_Hb1_ini,...
    Sigmas_Vb1_ini, grbm)
    ...
    CurrentSamples=normrnd( zeros(size(Weights_HbV_ini,2), grbm.nPTChains),...
    ...
    for iSamplingStep=1:nSamplingSteps
        for iPTChain=1:grbm.nPTChains
            ...
            CurrentSamples(:,iPTChain) = normrnd(v_mean, Sigmas_Vb1{iPTChain}(:,ones(size(CurrentSamples(:,iPTChain),2),1)));
        end
